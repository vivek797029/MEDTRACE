...
code(
    ...
    "\n",
    "# ── Load base model to CPU first, then try to move to GPU ────────────────\n",
    "# Loading to CPU avoids the CUDA device-side assert that occurs when the\n",
    "# GPU context is corrupted after a training crash. We then try .cuda();\n",
    "# if it fails (corrupted context) inference runs on CPU instead.\n",
    "_base = AutoModelForCausalLM.from_pretrained(\n",
    "    cfg.base_model,\n",
    "    torch_dtype    = torch.float16,\n",
    "    low_cpu_mem_usage = True,\n",
    "    # No device_map — always materialise on CPU first\n",
    ")\n",
    ...
    "        print(f'GPU unavailable ({type(_gpu_err).__name__}) — running on CPU (slower but works).\\n')\n",
    "else:\n",
