"""
Generates MedTrace_FL_Colab.ipynb

The notebook is a pure entry point:
  - clones github.com/vivek797029/MEDTRACE
  - installs from requirements.txt
  - imports and calls run_simulation() from the real codebase
  - zero inline training logic

Run:
    python3 build_colab_notebook.py
"""
import json

REPO_URL = "https://github.com/vivek797029/MEDTRACE.git"
REPO_DIR = "/content/MEDTRACE"
SRC_DIR  = f"{REPO_DIR}/src"

cells = []

def md(*lines):
    cells.append({"cell_type": "markdown", "metadata": {}, "source": list(lines)})

def code(*lines):
    cells.append({
        "cell_type": "code",
        "metadata": {},
        "execution_count": None,
        "outputs": [],
        "source": list(lines),
    })


# ─────────────────────────────────────────────────────────────────────────────
# Cell 0 — Title
# ─────────────────────────────────────────────────────────────────────────────
md(
    "# MedTrace FL — Federated Learning for Privacy-Preserving Medical AI\n",
    "\n",
    "This notebook is a **thin entry point** into the MedTrace codebase.\n",
    "All training logic lives in the GitHub repository — nothing is duplicated here.\n",
    "\n",
    "**What runs:**\n",
    "- `fl_simulate.run_simulation()` — full federated training loop\n",
    "- `fl_config.FLConfig` — typed, frozen configuration\n",
    "- `fl_adaptive_dp.AdaptiveDPMechanism` — novel adaptive per-client DP\n",
    "- `fl_evaluator` + `fl_plots` — evaluation and comparison plots\n",
    "\n",
    "**Auto-resume:** every round is checkpointed to Google Drive.\n",
    "If Colab disconnects, click **Runtime → Run all** — training picks up where it stopped.\n",
    "\n",
    "---\n",
    "## GPU unit cost (65 units)\n",
    "| GPU | Units / hr | 20 rounds (est.) |\n",
    "|-----|-----------|------------------|\n",
    "| T4  | 1.8       | 10–13 units ✅   |\n",
    "| L4  | 2.8       | 7–10 units  ✅   |\n",
    "| A100 | 9.6      | 18–22 units ⚠️  |\n",
    "\n",
    "> **Recommended:** Runtime → Change runtime type → **T4 GPU**\n",
)

# ─────────────────────────────────────────────────────────────────────────────
# Cell 1 — GPU + Drive
# ─────────────────────────────────────────────────────────────────────────────
md("## Step 1 — Check GPU and mount Google Drive")
code(
    "import os, torch\n",
    "\n",
    "# GPU check\n",
    "if not torch.cuda.is_available():\n",
    "    raise SystemExit(\n",
    "        'No GPU detected. '\n",
    "        'Go to Runtime → Change runtime type → T4 GPU, then Run all again.'\n",
    "    )\n",
    "gpu_name = torch.cuda.get_device_name(0)\n",
    "gpu_mem  = torch.cuda.get_device_properties(0).total_memory / 1e9\n",
    "print(f'GPU  : {gpu_name}')\n",
    "print(f'VRAM : {gpu_mem:.1f} GB')\n",
    "\n",
    "# Mount Drive for persistent checkpoints\n",
    "from google.colab import drive\n",
    "if not os.path.exists('/content/drive/MyDrive'):\n",
    "    drive.mount('/content/drive')\n",
    "print('Drive mounted.')\n",
)

# ─────────────────────────────────────────────────────────────────────────────
# Cell 2 — Clone + install
# ─────────────────────────────────────────────────────────────────────────────
md("## Step 2 — Clone repository and install dependencies")
code(
    "import subprocess, sys, os\n",
    "\n",
    f"REPO_URL = '{REPO_URL}'\n",
    f"REPO_DIR = '{REPO_DIR}'\n",
    f"SRC_DIR  = '{SRC_DIR}'\n",
    "\n",
    "# Clone or pull\n",
    "if os.path.exists(REPO_DIR):\n",
    "    print('Repository already present — pulling latest...')\n",
    "    subprocess.run(['git', '-C', REPO_DIR, 'pull', '--ff-only'], check=True)\n",
    "else:\n",
    "    print(f'Cloning {REPO_URL} ...')\n",
    "    subprocess.run(['git', 'clone', '--depth', '1', REPO_URL, REPO_DIR], check=True)\n",
    "\n",
    "# Install dependencies from requirements.txt\n",
    "req = os.path.join(REPO_DIR, 'requirements.txt')\n",
    "print('Installing dependencies...')\n",
    "subprocess.run(\n",
    "    [sys.executable, '-m', 'pip', 'install', '-q', '-r', req],\n",
    "    check=True,\n",
    ")\n",
    "\n",
    "# Add src/ to path so all project modules are importable\n",
    "if SRC_DIR not in sys.path:\n",
    "    sys.path.insert(0, SRC_DIR)\n",
    "\n",
    "# Confirm modules are importable\n",
    "import importlib\n",
    "for module in ['fl_config', 'fl_simulate', 'fl_adaptive_dp',\n",
    "               'fl_evaluator', 'fl_plots', 'fl_tracker']:\n",
    "    importlib.import_module(module)\n",
    "    print(f'  OK  {module}')\n",
    "\n",
    "print('Setup complete.')\n",
)

# ─────────────────────────────────────────────────────────────────────────────
# Cell 3 — Configuration
# ─────────────────────────────────────────────────────────────────────────────
md(
    "## Step 3 — Configure the experiment\n",
    "\n",
    "Edit this cell to change any hyperparameter.\n",
    "Everything uses the typed `FLConfig` dataclasses from the real codebase.\n",
)
code(
    "from fl_config import (\n",
    "    FLConfig, DPConfig, AdaptiveDPConfig,\n",
    "    LoRAConfig, TrainingConfig, EvalConfig,\n",
    "    TrackerConfig, HospitalRegistry,\n",
    ")\n",
    "\n",
    "# ── Paths (saved to your Google Drive) ───────────────────────────────────\n",
    "DRIVE_ROOT  = '/content/drive/MyDrive/MedTrace'\n",
    "CKPT_DIR    = f'{DRIVE_ROOT}/checkpoints'\n",
    "OUTPUT_DIR  = f'{DRIVE_ROOT}/outputs'\n",
    "\n",
    "os.makedirs(CKPT_DIR,  exist_ok=True)\n",
    "os.makedirs(OUTPUT_DIR, exist_ok=True)\n",
    "\n",
    "# ── Build config ──────────────────────────────────────────────────────────\n",
    "cfg = FLConfig(\n",
    "    # Model\n",
    "    base_model   = 'TinyLlama/TinyLlama-1.1B-Chat-v1.0',\n",
    "\n",
    "    # Federated learning\n",
    "    fl_rounds    = 20,      # full run; use 2 to smoke-test first\n",
    "    local_epochs = 1,\n",
    "    hospitals    = HospitalRegistry.build(3),   # 3 specialty hospitals\n",
    "\n",
    "    # LoRA adapter\n",
    "    lora = LoRAConfig(r=8, alpha=16, dropout=0.05),\n",
    "\n",
    "    # Differential privacy\n",
    "    dp = DPConfig(\n",
    "        enabled       = True,\n",
    "        epsilon       = 8.0,    # (ε, δ)-DP; lower = more private\n",
    "        delta         = 1e-5,\n",
    "        max_grad_norm = 1.0,\n",
    "    ),\n",
    "\n",
    "    # Adaptive per-client DP (novel research contribution)\n",
    "    adaptive_dp = AdaptiveDPConfig(\n",
    "        enabled              = True,\n",
    "        ema_alpha            = 0.1,   # gradient sensitivity smoothing\n",
    "        min_epsilon_fraction = 0.1,   # prevents any client from getting 0 budget\n",
    "    ),\n",
    "\n",
    "    # Training\n",
    "    training = TrainingConfig(\n",
    "        batch_size                  = 4,\n",
    "        gradient_accumulation_steps = 4,   # effective batch = 16\n",
    "        learning_rate               = 2e-4,\n",
    "        max_length                  = 512,\n",
    "    ),\n",
    "\n",
    "    # Evaluation (MCQ log-prob scoring, no generation needed)\n",
    "    eval = EvalConfig(\n",
    "        enabled             = True,\n",
    "        eval_every_n_rounds = 5,\n",
    "        num_eval_samples    = 200,\n",
    "    ),\n",
    "\n",
    "    # Experiment tracking ('none' | 'mlflow' | 'wandb')\n",
    "    tracker = TrackerConfig(backend='none'),\n",
    "\n",
    "    # Output goes to Drive\n",
    "    output_dir = OUTPUT_DIR,\n",
    ")\n",
    "\n",
    "# Print summary\n",
    "import math\n",
    "print('Experiment configuration')\n",
    "print(f'  Hospitals   : {cfg.num_hospitals}')\n",
    "for hid, h in cfg.hospitals.items():\n",
    "    print(f'    {hid}: {h.name} ({h.location})')\n",
    "print(f'  FL rounds   : {cfg.fl_rounds}')\n",
    "print(f'  Base model  : {cfg.base_model}')\n",
    "print(f'  LoRA        : r={cfg.lora.r}  alpha={cfg.lora.alpha}')\n",
    "print(f'  Batch       : {cfg.training.batch_size} x '\n",
    "      f'{cfg.training.gradient_accumulation_steps} = '\n",
    "      f'{cfg.training.batch_size * cfg.training.gradient_accumulation_steps} effective')\n",
    "if cfg.dp.enabled:\n",
    "    per_round_eps = cfg.dp.epsilon / math.sqrt(cfg.fl_rounds)\n",
    "    print(f'  DP          : enabled  ε={cfg.dp.epsilon}  δ={cfg.dp.delta}  '\n",
    "          f'σ={cfg.dp.sigma:.4f}  C={cfg.dp.max_grad_norm}')\n",
    "    print(f'  Per-round ε : {per_round_eps:.4f}  (advanced composition)')\n",
    "    print(f'  Adaptive DP : {cfg.adaptive_dp.enabled}')\n",
    "else:\n",
    "    print('  DP          : disabled (no-privacy baseline)')\n",
    "print(f'  Output      : {cfg.output_dir}')\n",
    "print(f'  Checkpoints : {CKPT_DIR}')\n",
)

# ─────────────────────────────────────────────────────────────────────────────
# Cell 4 — Train
# ─────────────────────────────────────────────────────────────────────────────
md(
    "## Step 4 — Run federated training\n",
    "\n",
    "Calls `run_simulation()` from `fl_simulate.py` in the repository.\n",
    "All logic — FedAvg, adaptive DP, evaluation, checkpointing — runs from source.\n",
)
code(
    "from fl_simulate import run_simulation, setup_logging\n",
    "import logging\n",
    "\n",
    "setup_logging(logging.INFO)\n",
    "\n",
    "report = run_simulation(\n",
    "    cfg,\n",
    "    checkpoint_dir = CKPT_DIR,\n",
    ")\n",
    "\n",
    "# Summary\n",
    "print()\n",
    "print('=' * 60)\n",
    "print('Training complete')\n",
    "print(f'  Time    : {report[\"total_training_time\"] / 60:.1f} min')\n",
    "print(f'  Rounds  : {len(report[\"round_metrics\"])}')\n",
    "if report['round_metrics']:\n",
    "    last = report['round_metrics'][-1]\n",
    "    print(f'  Loss    : {last[\"avg_loss\"]:.4f}')\n",
    "    print(f'  Diverge : {last[\"weight_divergence\"]:.6f}')\n",
    "if 'adaptive_dp_summary' in report:\n",
    "    print('  DP budget per hospital:')\n",
    "    for hid, s in report['adaptive_dp_summary']['per_client'].items():\n",
    "        pct = s['budget_spent'] / cfg.dp.epsilon * 100\n",
    "        print(f'    {hid}: {s[\"budget_spent\"]:.3f} / {cfg.dp.epsilon}  ({pct:.1f}%)')\n",
    "print('=' * 60)\n",
)

# ─────────────────────────────────────────────────────────────────────────────
# Cell 5 — Plots
# ─────────────────────────────────────────────────────────────────────────────
md(
    "## Step 5 — Generate plots\n",
    "\n",
    "Uses `fl_evaluator.EvalAccumulator` and `fl_plots.ResultsPlotter` from source.\n",
)
code(
    "import matplotlib\n",
    "matplotlib.use('Agg')\n",
    "import matplotlib.pyplot as plt\n",
    "from fl_evaluator import EvalAccumulator, EvalResult\n",
    "from fl_plots import ResultsPlotter\n",
    "\n",
    "# Populate accumulator from the training report\n",
    "acc = EvalAccumulator()\n",
    "acc.set_metadata(**{k: v for k, v in report['config'].items()\n",
    "                    if isinstance(v, (str, int, float, bool))})\n",
    "\n",
    "for rm in report['round_metrics']:\n",
    "    rn = rm['round']\n",
    "    acc.add_eval_result(EvalResult(\n",
    "        run_label            = 'MedTrace FL',\n",
    "        round_num            = rn,\n",
    "        accuracy             = 0.0,\n",
    "        loss                 = rm['avg_loss'],\n",
    "        perplexity           = 0.0,\n",
    "        num_eval_samples     = rm['total_samples'],\n",
    "        elapsed_seconds      = rm['aggregation_time'],\n",
    "        privacy_budget_spent = 0.0,\n",
    "    ))\n",
    "    for hid, m in rm.get('hospital_metrics', {}).items():\n",
    "        acc.add_client_metrics(hid, m)\n",
    "\n",
    "# Save and display plots\n",
    "plot_dir = os.path.join(OUTPUT_DIR, 'plots')\n",
    "os.makedirs(plot_dir, exist_ok=True)\n",
    "plotter  = ResultsPlotter(output_dir=plot_dir, fmt='png', dpi=120)\n",
    "paths    = plotter.save_all(acc)\n",
    "\n",
    "for path in paths:\n",
    "    img = plt.imread(path)\n",
    "    fig, ax = plt.subplots(figsize=(11, 4))\n",
    "    ax.imshow(img)\n",
    "    ax.axis('off')\n",
    "    plt.tight_layout()\n",
    "    plt.show()\n",
    "    print(f'Saved: {path}')\n",
    "\n",
    "# Also save full results JSON to Drive\n",
    "results_path = os.path.join(OUTPUT_DIR, 'results.json')\n",
    "acc.save_json(results_path)\n",
    "print(f'Results JSON: {results_path}')\n",
)

# ─────────────────────────────────────────────────────────────────────────────
# Cell 6 — Evaluation
# ─────────────────────────────────────────────────────────────────────────────
md(
    "## Step 6 — Evaluate the trained model\n",
    "\n",
    "Runs inference on clinical questions using the saved federated model.\n",
)
code(
    "from transformers import AutoModelForCausalLM, AutoTokenizer\n",
    "from peft import PeftModel\n",
    "\n",
    "device = 'cuda' if torch.cuda.is_available() else 'cpu'\n",
    "\n",
    "model_path = os.path.join(cfg.global_model_dir, f'round_{cfg.fl_rounds - 1}')\n",
    "print(f'Loading model from {model_path} ...')\n",
    "\n",
    "tokenizer = AutoTokenizer.from_pretrained(model_path)\n",
    "base      = AutoModelForCausalLM.from_pretrained(\n",
    "    cfg.base_model, torch_dtype=torch.float32)\n",
    "model     = PeftModel.from_pretrained(base, model_path)\n",
    "model.to(device).eval()\n",
    "print('Model ready.\\n')\n",
    "\n",
    "questions = [\n",
    "    'A 62-year-old man with hypertension has crushing chest pain radiating to '\n",
    "    'the left arm, diaphoresis, and nausea for 45 min. ECG: ST elevation in '\n",
    "    'II, III, aVF. What is the immediate management?',\n",
    "\n",
    "    'A 55-year-old woman has sudden left-sided weakness, facial droop, and '\n",
    "    'slurred speech for 2 hours. CT head: no hemorrhage. Next step?',\n",
    "\n",
    "    'A returned traveller from sub-Saharan Africa has cyclic fever, rigors, '\n",
    "    'and splenomegaly for 5 days. Blood smear: ring-form trophozoites. Treatment?',\n",
    "]\n",
    "\n",
    "for i, question in enumerate(questions, 1):\n",
    "    prompt = (\n",
    "        f'<|system|>\\n{cfg.system_msg}</s>\\n'\n",
    "        f'<|user|>\\n{question}</s>\\n'\n",
    "        f'<|assistant|>\\n'\n",
    "    )\n",
    "    inputs = tokenizer(prompt, return_tensors='pt').to(device)\n",
    "    with torch.no_grad():\n",
    "        output = model.generate(\n",
    "            **inputs,\n",
    "            max_new_tokens    = 400,\n",
    "            temperature       = 0.7,\n",
    "            do_sample         = True,\n",
    "            repetition_penalty= 1.1,\n",
    "            pad_token_id      = tokenizer.eos_token_id,\n",
    "        )\n",
    "    response = tokenizer.decode(\n",
    "        output[0][inputs['input_ids'].shape[1]:],\n",
    "        skip_special_tokens=True,\n",
    "    )\n",
    "    print(f'Q{i}: {question}')\n",
    "    print(f'A:  {response[:600]}')\n",
    "    print('-' * 70)\n",
    "\n",
    "del model, base\n",
    "if torch.cuda.is_available():\n",
    "    torch.cuda.empty_cache()\n",
)

# ─────────────────────────────────────────────────────────────────────────────
# Cell 7 — Interactive chat
# ─────────────────────────────────────────────────────────────────────────────
md("## Step 7 — Interactive chat (optional)")
code(
    "from transformers import AutoModelForCausalLM, AutoTokenizer\n",
    "from peft import PeftModel\n",
    "\n",
    "device     = 'cuda' if torch.cuda.is_available() else 'cpu'\n",
    "model_path = os.path.join(cfg.global_model_dir, f'round_{cfg.fl_rounds - 1}')\n",
    "\n",
    "tokenizer = AutoTokenizer.from_pretrained(model_path)\n",
    "base      = AutoModelForCausalLM.from_pretrained(\n",
    "    cfg.base_model, torch_dtype=torch.float32)\n",
    "model     = PeftModel.from_pretrained(base, model_path)\n",
    "model.to(device).eval()\n",
    "print('Ready. Type a clinical question (or \"quit\" to exit).\\n')\n",
    "\n",
    "while True:\n",
    "    question = input('Question: ').strip()\n",
    "    if question.lower() in ('quit', 'exit', 'q', ''):\n",
    "        break\n",
    "    prompt = (\n",
    "        f'<|system|>\\n{cfg.system_msg}</s>\\n'\n",
    "        f'<|user|>\\n{question}</s>\\n'\n",
    "        f'<|assistant|>\\n'\n",
    "    )\n",
    "    inputs = tokenizer(prompt, return_tensors='pt').to(device)\n",
    "    with torch.no_grad():\n",
    "        output = model.generate(\n",
    "            **inputs,\n",
    "            max_new_tokens    = 500,\n",
    "            temperature       = 0.7,\n",
    "            do_sample         = True,\n",
    "            repetition_penalty= 1.1,\n",
    "            pad_token_id      = tokenizer.eos_token_id,\n",
    "        )\n",
    "    answer = tokenizer.decode(\n",
    "        output[0][inputs['input_ids'].shape[1]:],\n",
    "        skip_special_tokens=True,\n",
    "    )\n",
    "    print(f'\\n{answer}\\n{\"─\" * 70}\\n')\n",
)

# ─────────────────────────────────────────────────────────────────────────────
# Write notebook
# ─────────────────────────────────────────────────────────────────────────────
notebook = {
    "nbformat": 4,
    "nbformat_minor": 0,
    "metadata": {
        "colab": {
            "provenance": [],
            "gpuType": "T4",
            "toc_visible": True,
        },
        "kernelspec": {"display_name": "Python 3", "name": "python3"},
        "language_info": {"name": "python"},
        "accelerator": "GPU",
    },
    "cells": cells,
}

OUT = "/sessions/magical-serene-newton/mnt/Downloads/medtrace/MedTrace_FL_Colab.ipynb"
with open(OUT, "w") as f:
    json.dump(notebook, f, indent=1)

print(f"Written : {OUT}")
print(f"Cells   : {len(cells)}")
