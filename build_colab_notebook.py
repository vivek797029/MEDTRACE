...
code(
    ...
    "        _report_ok = True\n",
    "    else:\n",
    "        print('⚠️  fl_training_report.json not found on Drive.')\n",
    "        print('    This happens when training crashed before the metrics were saved.')\n",
    "        print('    Your trained model files ARE still there — go to Step 6 to run inference.')\n",
    "        print('    To get plots: re-run the training cell (it will resume from the last checkpoint).')\n",
    "        report = None\n",
    "else:\n",
    "    _report_ok = (report is not None)\n",
    "\n",
    "if _report_ok and report is not None:\n",
    "    import matplotlib\n",
    "    matplotlib.use('Agg')\n",
    ...
    "    acc = EvalAccumulator()\n",
    "    _cfg_meta = report.get('config', {})\n",
    "    acc.set_metadata(**{k: v for k, v in _cfg_meta.items()\n",
    "                        if isinstance(v, (str, int, float, bool))})\n",
    "\n",
    "    # Per-round eval accuracy (if available)\n",
    "    eval_by_round = {\n",
    ...
    "    }\n",
    "\n",
    "    # Support both key names: 'round_metrics' (server) or 'all_round_metrics' (simulate)\n",
    "    _rmetrics = report.get('round_metrics') or report.get('all_round_metrics', [])\n",
    "    for rm in _rmetrics:\n",
    "        rn = rm.get('round', rm.get('round_num', 0))\n",
    "        acc.add_eval_result(EvalResult(\n",
    "            run_label            = 'MedTrace FL',\n",
    "            round_num            = rn,\n",
    "            accuracy             = eval_by_round.get(rn, 0.0),\n",
    "            loss                 = rm.get('avg_loss', rm.get('loss', 0.0)),\n",
    "            perplexity           = 0.0,\n",
    "            num_eval_samples     = rm.get('total_samples', rm.get('num_samples', 0)),\n",
    "            elapsed_seconds      = rm.get('aggregation_time', rm.get('elapsed', 0.0)),\n",
    "            privacy_budget_spent = 0.0,\n",
    "        ))\n",
    "        for hid, m in rm.get('hospital_metrics', rm.get('client_metrics', {})).items():\n",
...
code(
    ...
    "\n",
    "# ── Load entirely on CPU using safetensors device='cpu' ──────────────────\n",
    "# PeftModel.from_pretrained internally calls safetensors without forcing\n",
    "# device='cpu', which dequeues the CUDA error left over from training.\n",
    "# We bypass it: load base on CPU, build the PEFT wrapper, then inject\n",
    "# adapter weights via safetensors.load_file(device='cpu') — zero GPU calls.\n",
    "_base = AutoModelForCausalLM.from_pretrained(\n",
    "    cfg.base_model,\n",
    "    torch_dtype       = torch.float32,  # float32 on CPU avoids fp16 CPU slowness\n",
    "    low_cpu_mem_usage = True,\n",
    ")\n",
    "_peft_config = PeftConfig.from_pretrained(_target)\n",
    "_model = get_peft_model(_base, _peft_config)\n",
    "\n",
    "# Load adapter weights with device explicitly forced to CPU\n",
    "_adapter_file = _os.path.join(_target, 'adapter_model.safetensors')\n",
    "if _os.path.exists(_adapter_file):\n",
    "    _adapter_weights = _st.load_file(_adapter_file, device='cpu')\n",
    "    _missing, _unexpected = _model.load_state_dict(_adapter_weights, strict=False)\n",
    "    print(f'Adapter loaded (missing={len(_missing)}, unexpected={len(_unexpected)})')\n",
    "else:\n",
    "    # Fall back to .bin weights if no safetensors\n",
    "    _bin_file = _os.path.join(_target, 'adapter_model.bin')\n",
    "    if _os.path.exists(_bin_file):\n",
    "        _adapter_weights = torch.load(_bin_file, map_location='cpu')\n",
    "        _model.load_state_dict(_adapter_weights, strict=False)\n",
    "        print('Adapter loaded from .bin')\n",
    "    else:\n",
    "        print('Warning: no adapter weights file found — using base model only')\n",
    "\n",
    "_model.eval()\n",
    "\n",
    "# ── Try to move to GPU now that all loading is done ───────────────────────\n",
    "_device = 'cpu'\n",
    "try:\n",
    "    if torch.cuda.is_available():\n",
    "        _model = _model.half().cuda()  # Convert to fp16 for GPU efficiency\n",
    "        _device = 'cuda'\n",
    "        print('Model ready on GPU (fp16).\\n')\n",
    "    else:\n",
    "        print('Model ready on CPU (fp32). Inference ~1-2 min/question.\\n')\n",
    "except Exception as _gpu_err:\n",
    "    _device = 'cpu'\n",
    "    print(f'GPU move failed — running on CPU. Inference ~1-2 min/question.\\n')\n",
    "\n",
